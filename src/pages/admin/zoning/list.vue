<template>
  <div class="vApplication__page">
    ...
    <div ref="isHook__Name">
      / {{$t('CONTROL.LABEL').toUpperCase()}} / {{$t('CONTROL.ZONING').toUpperCase()}}
    </div>
    ...
    <!--ADD FORM-->
      <v-dialog v-model="dialog_addNewForm" persistent scrollable max-width="600">
        <editForm
          ...
          v-on:success-dialog="uploadFile"
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        nameKeySelected: 'id',
        headers: [
          {id: 'name', text: this.$t('TABLE.' + key.toUpperCase()  + '.name') , type: "file"},
