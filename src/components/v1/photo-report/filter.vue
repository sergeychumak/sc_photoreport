...
export default {
  ...
  mounted: function(){
    Promise.all([
      ...
      this.fillShops()
        ...
        Object.keys(this.$route.query).map(el=>{
          if (this.$route.query[el]!==null){
            f = true
          }
        })

        if (f){
          this.linesCurrentSelected = parseInt(this.$route.query.line) || null
          this.categoriesCurrentSelected = parseInt(this.$route.query.category) || null
          this.regionsCurrentSelected = parseInt(this.$route.query.region) || null
          this.departmentsCurrentSelected = parseInt(this.$route.query.department) || null
          this.shopsCurrentSelected = parseInt(this.$route.query.shop) || null
          this.FACETS_GET_LIST(this.resultData)
        }
  ...
  methods: {
    ...
    ...mapActions( "facets",      { FACETS_GET_LIST:      "get_list" }),
    ...mapActions( "images",      { CLEAR_LIST:           "clear_list" }),
    ...
    apply: function(){
      this.CLEAR_LIST()
      this.FACETS_GET_LIST(this.resultData).then(()=>{
        this.$router.push({ name: "show-photoreport", query: this.resultData })
      })
    }
  },
  computed: {
